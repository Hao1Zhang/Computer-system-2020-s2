...
namespace Workshop_Tokeniser
{
    ...
    enum Char : int
    {
        cg_min = 128,           // make all groups start after 128
                                ...
                                // all characters used in the tokeniser's grammar
        cg_start_of_token,      // ('a'-'z') | ('A'-'Z') | ('0'-'9') | '+' | '-' | '*' | '/' |
                                // '<' | '=' | '!' | '>' | '@' | '{' | '}' | '(' | ')' | ':' | ';' | '.' | ',' | '"'

                                // any wspace character
        cg_wspace,              // '\n' | ' '

                                // internal part of an integer
        cg_start_of_integer,    // '0'-'9'
        cg_extends_integer,     // '0'-'9'

                                // internal part of an identifier
        cg_start_of_identifier, // ('a'-'z') | ('A'-'Z')
    ...
    // it's spelling contains all but one of the characters read by nextch() since the last call
