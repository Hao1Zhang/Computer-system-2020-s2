...
void walk_class_var_decs(ast t)
{
    int counter = size_of_class_var_decs(t) ;
    //output
    for ( int i = 0 ; i < counter ; i++ )
    {
        walk_var_dec(get_class_var_decs(t,i)) ;
    }
    for (int i = 0; i < static_variable.size(); ++i)
    {
        write_to_output(add_space()+static_variable[i]+"\n");
    }
    
    if (static_variable.size() > 0 )
    {
        write_to_output("\n");
    }
    
    for (int i = 0; i < field_variable.size(); ++i)
    {
        write_to_output(add_space()+field_variable[i]+"\n");
    }
...
void walk_var_dec(ast t)
{
    ...
    {
        static_counter++;
        static_variable.push_back("static " + type + " " + name + " ;" );
    }
    if (segment == "argument")
    {
    ...
    {
        field_counter++;
        field_variable.push_back("field " + type + " " + name + " ;");
    }
    if (segment == "local")
    {
        write_to_output(add_space()+"var " + type +" "+ name +" ;\n");
    }
}
...
void walk_constructor(ast t)
{
    ...
    //output
    write_to_output(add_space()+"constructor "+ vtype +" "+ name+"(");
...
void walk_function(ast t)
{
    ...
    //output
    write_to_output(add_space() +"function "+ vtype +" "+ name +"(") ;
...
void walk_method(ast t)
{
    ...
    //output
    write_to_output(add_space()+"method " + vtype + " " + name + "(") ;
    walk_param_list(param_list) ;
    write_to_output(")\n") ;
...
void walk_param_list(ast t)
{
    int counter = size_of_param_list(t) ;
    for ( int i = 0 ; i < counter ; i++ )
    {
...
void walk_var_decs(ast t)
{
    int counter = size_of_var_decs(t) ;
    for ( int i = 0 ; i < counter ; i++ )
    {
...
void walk_statements(ast t)
{
    int count = size_of_statements(t) ;
...
void walk_if_else(ast t)
{
    ...
    space_counter-- ;
    write_to_output(add_space() + "}\n");
    write_to_output(add_space() + "else\n");
    write_to_output(add_space() + "{\n");
    space_counter++ ;
...
void walk_while(ast t)
{
    ...
    //output
    write_to_output(add_space()+"while (") ;
    walk_expr(condition) ;
    write_to_output(")\n") ;
    write_to_output(add_space()+"{\n") ;
    space_counter++ ;
...
void walk_return(ast t)
{
    write_to_output(add_space()+"return ;\n") ;
}
