...
string myclassname;
int method_symbol = false;
int constructor_symbol = false;
...
void walk_constructor(ast t)
{
    ...
    constructor_symbol = true;
    int var_number = size_of_var_decs(var);

    write_to_output("function "+myclassname+"."+name+" "+to_string(var_number)+"\n");
    walk_param_list(param_list);
    walk_subr_body(subr_body);
    constructor_symbol = false;
}
...
void walk_subr_body(ast t)
{
    ...
    {
        write_to_output("push constant "+to_string(counter)+"\n");
...
void walk_if(ast t)
{
    string count = to_string(if_count++);
    ast condition = get_if_condition(t);
    ast if_true = get_if_if_true(t);

    walk_expr(condition);
    write_to_output("if-goto IF_TRUE"+count+"\n");
    write_to_output("goto IF_FALSE"+count+"\n");
    write_to_output("label IF_TRUE"+count+"\n");
    walk_statements(if_true);
    write_to_output("label IF_FALSE"+count+"\n");
}
...
void walk_if_else(ast t)
{
    ...
    write_to_output("label IF_TRUE"+count+"\n");
    walk_statements(if_true);
    write_to_output("goto IF_END"+count+"\n");
    write_to_output("label IF_FALSE"+count+"\n");
    walk_statements(if_false);
    write_to_output("label IF_END"+count+"\n");
}
...
void walk_bool(ast t)
{
    int _constant = get_bool_t_or_f(t);
